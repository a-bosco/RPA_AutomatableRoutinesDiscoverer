...
public class WFTreeHandler {
	 ...
	 * @return position of a node in a parent sequence (S) node starting attributes 0, or -1 if order is not defined for this node
	 */
...
//		System.out.println(children.indexOf(pathB.get(1)));
	...
	public boolean areInterleaving(Node t1, Node t2) {
		...
		// Get path attributes ROOT to gamma
		List<IRPSTNode<Flow, Node>> path = this.wfTree.getDownwardPath(this.wfTree.getRoot(), gamma);
		
		// check path attributes ROOT to the parent of gamma
		for (int i=0; i<path.size()-1; i++) { 
	...
	public boolean areCooccurring(Node t1, Node t2) {
		...
		// check path attributes gamma to beta
	...
	/**
	 * Check if child fragment is in some loop within a parent fragment, i.e., there exists path attributes exit to entry of the child fragment
