...
public class ProcessSerializationTest extends TestCase {
	...
	public void testJSON2Process() {
		...
		assertEquals(process.getName(), "Main case");
	...
	public void testProcess2JSON() {
		ProcessModel process = new ProcessModel("Main case");
	...
	public void testSerializationException() {
		String json = "{'name' : 'Main case', 'tasks' : [], 'gateways' : []}";
		try {
	...
	public void testWrongGatewayType() {
		String json = "{'name' : 'Main case', 'tasks' : [], 'gateways' : [{'id' : 'gate1', type : 'FOR'}], 'flows' : []}";
		try {
			JSON2Process.convert(json);
			assertTrue("Should throw exception.", false);
		} catch (Exception e) {
