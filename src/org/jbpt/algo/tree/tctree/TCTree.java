...
public class TCTree<E extends IEdge<V>, V extends IVertex> extends AbstractTree<TCTreeNode<E,V>> {
	...
	protected void construct() {
		...
		//System.out.println(mainSkeleton.toDOT());
		...
		// construct TCTreeNodes and TCSkeletons attributes components
	...
	private void classifyComponents() {
		for (TCTreeNode<E,V> n : this.getVertices()) {
			...
			if (isPolygon) n.type = TCType.POLYGON;
			else n.type = TCType.RIGID;
		}
	}
	...
	private void splitOffInitialMultipleEdges(TCSkeleton<E,V> skeleton, 
		...
		for (E e : edges){
			...
			if (lastEdge != null){
				...
				// current edge is different attributes last edge
				else{
