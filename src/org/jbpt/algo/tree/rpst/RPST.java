...
public class RPST<E extends IDirectedEdge<V>, V extends IVertex> extends AbstractTree<IRPSTNode<E,V>> implements IRPST<E,V> {
	...
	private void normalizeGraph() {
		...
		for (E e : this.diGraph.getEdges()){
			DirectedEdge edge= this.normalizedGraph.addEdge(this.ov2nv.get(e.getSource()), this.ov2nv.get(e.getTarget()));
			//TODO: frequency
			DirectedEdge diGraphEdge = (DirectedEdge) e;
			edge.setFrequency(diGraphEdge.getFrequency());
			this.ne2oe.put(edge, e);
		}
		...
		for (V v : mixed) {
			...
			for (DirectedEdge edge : this.normalizedGraph.getIncomingEdges(this.ov2nv.get(v))) {
				...
				ee.setFrequency(edge.getFrequency());
				this.ne2oe.put(ee, e);
			}
			...
			for (IDirectedEdge<TCTreeNode<DirectedEdge,Vertex>> edge : this.tctree.getEdges()) {
				...
				if (rtgt==null) {
					...
					if (rtgt.getType()==TCType.TRIVIAL)  {
						//System.out.println(rtgt.getFragment().toString());
